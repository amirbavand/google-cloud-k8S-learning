    ...
    async componentDidMount() {
      ...
      async checkLoggedInGetImage() {
        try {
          ...
          const values = await axios.get('/api/originalphoto',{ headers:{'x-access-token': this.state.xtoken, "productId":product_id,  "imageId":image_id}});

          const imgstr=(values.data.imagestr);
          console.log(values.data);
          

          this.setState({file: "data:image/png;base64,".concat(imgstr)  });
